...
export default function TeamDetailScreen({ navigation, route }) {
  ...
  const [joinRequests, setJoinRequests] = useState([
    { id: 1, user: '李明', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=user1', reason: '我对Python很感兴趣，希望能加入团队一起学习', time: '2小时前', type: 'join', status: 'pending' },
    { id: 2, user: '王芳', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=user2', reason: '想和大家一起交流学习经验', time: '5小时前', type: 'join', status: 'pending' },
  ]);
  const [adminRequests, setAdminRequests] = useState([
    { id: 3, user: '张伟', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=user3', reason: '我有3年Python开发经验，可以帮助团队成员解决技术问题，组织学习活动', time: '1天前', type: 'admin', status: 'pending' },
  ]);

  const tabs = getTabsForRole(team.isAdmin);

  // 可邀请的用户列表
  const platformUsers = [
    { id: 1, name: '李明', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=user1', title: 'Python开发者', verified: true },
    { id: 2, name: '王芳', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=user2', title: 'Web工程师', verified: false },
    { id: 3, name: '刘强', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=user3', title: '全栈开发', verified: true },
    { id: 4, name: '陈静', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=user4', title: '前端专家', verified: true },
    { id: 5, name: '张伟', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=user5', title: '后端工程师', verified: false },
  ];

  const twitterUsers = [
    { id: 't1', name: '@pythondev', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=twitter1', followers: '10K', verified: true },
    { id: 't2', name: '@webmaster', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=twitter2', followers: '5K', verified: false },
    { id: 't3', name: '@coder_life', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=twitter3', followers: '8K', verified: true },
  ];

  const facebookUsers = [
    { id: 'f1', name: 'Python Learning Group', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=fb1', members: '2K', verified: true },
    { id: 'f2', name: 'Web Dev Community', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=fb2', members: '1.5K', verified: false },
    { id: 'f3', name: 'Coding Together', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=fb3', members: '3K', verified: true },
  ];
  ...
  const handleApproveAdmin = (requestId, approve) => {
    ...
    if (approve) {
      Alert.alert('成功', `已同意 ${request.user} 成为管理员`);
      setAdminRequests(adminRequests.filter(r => r.id !== requestId));
    } else {
      Alert.alert('已拒绝', `已拒绝 ${request.user} 的管理员申请`);
      setAdminRequests(adminRequests.filter(r => r.id !== requestId));
    }
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>{restrictedView ? '团队成员' : '团队详情页'}</Text>
        </View>
        {!restrictedView && (
        ...
        {restrictedView && <View style={{ width: 24 }} />}
      </View>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {restrictedView && (
          <View style={styles.restrictedNotice}>
            ...
            <Text style={styles.restrictedNoticeText}>您还未加入该团队，仅可查看团队成员信息</Text>
          </View>
        )}
            ...
            <View style={styles.teamTitleRow}>
              ...
              {team.isAdmin && (
                ...
                >
                  ...
                  <Text style={styles.compactDismissBtnText}>解散</Text>
                </TouchableOpacity>
              )}
              {/* 普通成员：显示三个操作按钮 */}
              {!team.isAdmin && team.currentUserId !== team.creatorId && (
                <View style={styles.compactActionsRow}>
                  ...
                  >
                    ...
                    <Text style={styles.compactActionBtnText}>活动</Text>
                  </TouchableOpacity>
                  <TouchableOpacity 
                    style={[styles.compactActionBtn, styles.compactActionBtnPurple, hasApplied && styles.compactActionBtnDisabled]}
                    onPress={() => !hasApplied && setShowApplyAdminModal(true)}
                    disabled={hasApplied}
                  >
                  ...
                  <TouchableOpacity 
                    style={[styles.compactActionBtn, styles.compactActionBtnRed]}
                    onPress={handleExitTeam}
                  >
                    <Ionicons name="exit-outline" size={14} color="#ef4444" />
                    <Text style={[styles.compactActionBtnText, styles.compactActionBtnTextRed]}>退出</Text>
                  </TouchableOpacity>
                </View>
              )}
            </View>
          ...
          {restrictedView ? (
            <View style={styles.teamMembersGrid}>
              {teamMembers.map(member => (
                <View key={member.id} style={styles.teamMemberGridItem}>
              ...
              <ScrollView style={styles.approvalList} showsVerticalScrollIndicator={false}>
                ...
                {joinRequests.length > 0 && (
                  <View style={styles.approvalSection}>
                    <Text style={styles.approvalSectionTitle}>加入团队申请 ({joinRequests.length})</Text>
                    {joinRequests.map(request => (
                      <View key={request.id} style={styles.approvalItem}>
                        ...
                        <View style={styles.approvalReasonBox}>
                          <Text style={styles.approvalReasonLabel}>申请理由：</Text>
                          <Text style={styles.approvalReasonText}>{request.reason}</Text>
                        </View>
                        <View style={styles.approvalActions}>
                          <TouchableOpacity 
                            style={styles.approvalRejectBtn}
                            onPress={() => handleApproveJoin(request.id, false)}
                          >
                            <Ionicons name="close-circle" size={18} color="#ef4444" />
                            <Text style={styles.approvalRejectText}>拒绝</Text>
                          </TouchableOpacity>
                          <TouchableOpacity 
                            style={styles.approvalApproveBtn}
                            onPress={() => handleApproveJoin(request.id, true)}
                          >
                            <Ionicons name="checkmark-circle" size={18} color="#fff" />
                            <Text style={styles.approvalApproveText}>同意</Text>
                          </TouchableOpacity>
                        </View>
                      </View>
                    ))}
                ...
                {adminRequests.length > 0 && (
                  <View style={styles.approvalSection}>
                    <Text style={styles.approvalSectionTitle}>管理员申请 ({adminRequests.length})</Text>
                    {adminRequests.map(request => (
                      <View key={request.id} style={styles.approvalItem}>
                          ...
                          <View style={styles.approvalUserInfo}>
                            <View style={styles.approvalUserNameRow}>
                              ...
                              <View style={styles.adminApplyBadge}>
                                ...
                                <Text style={styles.adminApplyBadgeText}>申请管理员</Text>
                              </View>
                            </View>
                            <Text style={styles.approvalTime}>{request.time}</Text>
                          </View>
                        ...
                        <View style={styles.approvalActions}>
                          <TouchableOpacity 
                            style={styles.approvalRejectBtn}
                            onPress={() => handleApproveAdmin(request.id, false)}
                          >
                            <Ionicons name="close-circle" size={18} color="#ef4444" />
                            <Text style={styles.approvalRejectText}>拒绝</Text>
                          </TouchableOpacity>
                          <TouchableOpacity 
                            style={styles.approvalApproveBtn}
                            onPress={() => handleApproveAdmin(request.id, true)}
                          >
                            <Ionicons name="checkmark-circle" size={18} color="#fff" />
                            <Text style={styles.approvalApproveText}>同意</Text>
                          </TouchableOpacity>
                ...
                {/* 空状态 */}
                {joinRequests.length === 0 && adminRequests.length === 0 && (
                  <View style={styles.approvalEmpty}>
                    <Ionicons name="checkmark-done-circle-outline" size={64} color="#d1d5db" />
                    <Text style={styles.approvalEmptyText}>暂无待审批消息</Text>
                    <Text style={styles.approvalEmptyHint}>所有申请都已处理完毕</Text>
                  </View>
                )}
              </ScrollView>
            )}
      ...
      {/* 底部输入栏 - 限制访问模式下不显示 */}
      {!restrictedView && isJoined && activeTab === '团队讨论' && (
        ...
        <View style={styles.teamActions}>
          {isPending ? (
            <View style={styles.pendingNotice}>
              ...
              <Text style={styles.pendingNoticeText}>申请审核中，需要50%以上成员同意</Text>
            </View>
          ) : (
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.inviteModal}>
            ...
            <View style={styles.inviteModalHeader}>
              <Text style={styles.inviteModalTitle}>邀请用户加入团队</Text>
              <Text style={styles.inviteModalSubtitle}>选择用户并发送邀请</Text>
            </View>

            {/* 标签栏 */}
            <View style={styles.inviteTabBar}>
              <TouchableOpacity
                style={[styles.inviteTab, inviteTab === 'platform' && styles.inviteTabActive]}
                onPress={() => {
              ...
              >
                ...
                <Text style={[styles.inviteTabText, inviteTab === 'platform' && styles.inviteTabTextActive]}>
                  本站用户
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.inviteTab, inviteTab === 'twitter' && styles.inviteTabActive]}
                onPress={() => {
              ...
              >
                ...
                <Text style={[styles.inviteTabText, inviteTab === 'twitter' && styles.inviteTabTextActive]}>
                  Twitter
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.inviteTab, inviteTab === 'facebook' && styles.inviteTabActive]}
                onPress={() => {
              ...
              >
                ...
                <Text style={[styles.inviteTabText, inviteTab === 'facebook' && styles.inviteTabTextActive]}>
                  Facebook
                </Text>
              </TouchableOpacity>
            </View>
            ...
            <ScrollView style={styles.inviteUserList} showsVerticalScrollIndicator={false}>
              {getCurrentUserList().map(user => {
                ...
                return (
                  <TouchableOpacity
                    key={user.id}
                    style={[styles.inviteUserItem, isSelected && styles.inviteUserItemSelected]}
                    onPress={() => handleToggleUser(user)}
                  >
                    <View style={styles.inviteUserLeft}>
                      <Avatar uri={user.avatar} size={40} />
                      <View style={styles.inviteUserInfo}>
                        <View style={styles.inviteUserNameRow}>
            ...
            <View style={styles.inviteModalFooter}>
              ...
              >
                <Text style={styles.cancelInviteBtnText}>取消</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.sendInviteBtn, selectedUsers.length === 0 && styles.sendInviteBtnDisabled]}
                onPress={handleSendInvite}
                disabled={selectedUsers.length === 0}
              >
                <Ionicons name="send" size={16} color="#fff" />
                <Text style={styles.sendInviteBtnText}>发送邀请</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.applyAdminModal}>
            ...
            <View style={styles.applyAdminModalHeader}>
              <Text style={styles.applyAdminModalTitle}>申请成为管理员</Text>
            ...
            <ScrollView style={styles.applyAdminModalContent} showsVerticalScrollIndicator={false}>
              <View style={styles.applyAdminSection}>
                <Text style={styles.applyAdminLabel}>申请理由 <Text style={{ color: '#ef4444' }}>*</Text></Text>
                <TextInput
                  style={styles.applyAdminTextarea}
                  placeholder="请说明您申请成为管理员的理由，例如：您的经验、能为团队做什么等..."
                  placeholderTextColor="#9ca3af"
                  value={applyReason}
              ...
              <View style={styles.applyAdminTips}>
                <Text style={styles.applyAdminTipsTitle}>温馨提示</Text>
                <Text style={styles.applyAdminTipsText}>• 管理员可以邀请用户、发布公告、发起活动</Text>
                <Text style={styles.applyAdminTipsText}>• 申请需要团队创建者审核通过</Text>
                <Text style={styles.applyAdminTipsText}>• 请认真填写申请理由，提高通过率</Text>
              </View>
            </ScrollView>
            ...
            <View style={styles.applyAdminModalFooter}>
              ...
              >
                <Text style={styles.cancelApplyBtnText}>取消</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.submitApplyBtn, !applyReason.trim() && styles.submitApplyBtnDisabled]}
                onPress={handleApplyAdmin}
                disabled={!applyReason.trim()}
              >
                <Text style={styles.submitApplyBtnText}>提交申请</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.dismissModal}>
            ...
            <View style={styles.dismissModalHeader}>
              ...
              <Text style={styles.dismissModalTitle}>解散团队</Text>
            ...
            <View style={styles.dismissModalContent}>
              <View style={styles.dismissWarningBox}>
                <Text style={styles.dismissWarningTitle}>⚠️ 解散后将发生以下情况：</Text>
                <Text style={styles.dismissWarningText}>• 团队所有数据将被永久删除</Text>
                <Text style={styles.dismissWarningText}>• 所有成员将被移出团队</Text>
                <Text style={styles.dismissWarningText}>• 团队讨论记录将无法恢复</Text>
                <Text style={styles.dismissWarningText}>• 团队公告将全部删除</Text>
              </View>

              <View style={styles.dismissConfirmSection}>
                <Text style={styles.dismissConfirmLabel}>
                  请输入团队名称 <Text style={styles.dismissTeamNameHighlight}>"{team.name}"</Text> 以确认解散
                </Text>
                <TextInput
                  style={styles.dismissConfirmInput}
                  placeholder="请输入团队名称"
                  placeholderTextColor="#9ca3af"
                  value={dismissConfirmText}
                  onChangeText={setDismissConfirmText}
                />
              </View>
            </View>
            ...
            <View style={styles.dismissModalFooter}>
              ...
              >
                <Text style={styles.cancelDismissBtnText}>取消</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.confirmDismissBtn, dismissConfirmText !== team.name && styles.confirmDismissBtnDisabled]}
                onPress={handleConfirmDismiss}
                disabled={dismissConfirmText !== team.name}
              >
                <Ionicons name="trash" size={18} color="#fff" />
                <Text style={styles.confirmDismissBtnText}>确认解散</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  headerActions: { flexDirection: 'row', alignItems: 'center', gap: 8, zIndex: 10 },
  iconBtn: { width: 40, height: 40, alignItems: 'center', justifyContent: 'center', borderRadius: 20, backgroundColor: '#f9fafb' },
  moreActionsBtn: { width: 40, height: 40, alignItems: 'center', justifyContent: 'center', borderRadius: 20, backgroundColor: '#f9fafb' },
  content: { flex: 1 },
  teamInfoCard: { backgroundColor: '#fff', padding: 16, borderBottomWidth: 1, borderBottomColor: '#f3f4f6' },
  teamTitleRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 },
  teamName: { fontSize: 18, fontWeight: '600', color: '#1f2937', flex: 1, marginRight: 8 },
  // 紧凑型操作按钮（放在标题右侧）
  compactActionsRow: { flexDirection: 'row', gap: 6 },
  compactActionBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, backgroundColor: '#eff6ff', borderWidth: 1, borderColor: '#bfdbfe' },
  compactActionBtnPurple: { backgroundColor: '#f5f3ff', borderColor: '#ddd6fe' },
  compactActionBtnRed: { backgroundColor: '#fef2f2', borderColor: '#fecaca' },
  compactActionBtnDisabled: { backgroundColor: '#f3f4f6', borderColor: '#e5e7eb' },
  compactActionBtnText: { fontSize: 11, color: '#3b82f6', fontWeight: '600' },
  compactActionBtnTextPurple: { color: '#8b5cf6' },
  compactActionBtnTextRed: { color: '#ef4444' },
  compactActionBtnTextDisabled: { color: '#9ca3af' },
  compactDismissBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, backgroundColor: '#fef2f2', borderWidth: 1, borderColor: '#fecaca' },
  compactDismissBtnText: { fontSize: 11, color: '#ef4444', fontWeight: '600' },
  ...
  // 网格模式（未加入团队）
  teamMembersGrid: { flexDirection: 'row', flexWrap: 'wrap', paddingHorizontal: 8 },
  teamMemberGridItem: { alignItems: 'center', width: '20%', marginBottom: 16 },
  // 通用样式
  ...
  teamMemberName: { fontSize: 12, color: '#1f2937', fontWeight: '500', marginBottom: 2, textAlign: 'center', width: '100%', paddingHorizontal: 2 },
  ...
  tabBadge: { position: 'absolute', top: 6, right: '25%', backgroundColor: '#ef4444', minWidth: 18, height: 18, borderRadius: 9, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 4 },
  tabBadgeText: { fontSize: 10, color: '#fff', fontWeight: '600' },
  // 团队聊天区域 - 参照团队弹窗样式
  ...
  // 限制访问提示
  restrictedNotice: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#fef3c7', paddingVertical: 12, paddingHorizontal: 16, gap: 8, borderBottomWidth: 1, borderBottomColor: '#fde68a' },
  restrictedNoticeText: { fontSize: 13, color: '#92400e', fontWeight: '500', flex: 1 },
  pendingNotice: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#fef3c7', paddingVertical: 14, borderRadius: 12, gap: 8 },
  pendingNoticeText: { fontSize: 14, color: '#92400e', fontWeight: '500' },
  // 公告列表
  ...
  // 审批消息列表
  approvalList: { flex: 1, backgroundColor: '#f9fafb' },
  approvalSection: { paddingHorizontal: 16, paddingTop: 16 },
  approvalSectionTitle: { fontSize: 15, fontWeight: '600', color: '#374151', marginBottom: 12 },
  approvalItem: { backgroundColor: '#fff', borderRadius: 12, padding: 16, marginBottom: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, elevation: 2 },
  approvalHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 12 },
  approvalUserInfo: { flex: 1, marginLeft: 12 },
  approvalUserNameRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 4 },
  approvalUserName: { fontSize: 15, fontWeight: '600', color: '#1f2937' },
  approvalTime: { fontSize: 12, color: '#9ca3af' },
  adminApplyBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#f5f3ff', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 10 },
  adminApplyBadgeText: { fontSize: 11, color: '#8b5cf6', fontWeight: '500' },
  approvalReasonBox: { backgroundColor: '#f9fafb', borderRadius: 8, padding: 12, marginBottom: 12 },
  approvalReasonLabel: { fontSize: 12, color: '#6b7280', fontWeight: '500', marginBottom: 4 },
  approvalReasonText: { fontSize: 13, color: '#374151', lineHeight: 20 },
  approvalActions: { flexDirection: 'row', gap: 12 },
  approvalRejectBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: '#fef2f2', paddingVertical: 10, borderRadius: 8, borderWidth: 1, borderColor: '#fecaca' },
  approvalRejectText: { fontSize: 14, color: '#ef4444', fontWeight: '600' },
  approvalApproveBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: '#22c55e', paddingVertical: 10, borderRadius: 8 },
  approvalApproveText: { fontSize: 14, color: '#fff', fontWeight: '600' },
  approvalEmpty: { alignItems: 'center', justifyContent: 'center', paddingVertical: 80 },
  ...
  // 申请管理员按钮
  applyAdminBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#f5f3ff', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8 },
  applyAdminBtnDisabled: { backgroundColor: '#f3f4f6' },
  applyAdminBtnText: { fontSize: 13, color: '#8b5cf6', fontWeight: '500' },
  ...
  dismissWarningText: { fontSize: 13, color: '#dc2626', lineHeight: 22, marginBottom: 4 },
