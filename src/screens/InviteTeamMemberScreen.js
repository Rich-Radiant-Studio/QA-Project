...
export default function InviteTeamMemberScreen({ navigation, route }) {
  ...
  // 模拟用户数据
  const platformUsers = [
    { id: 1, name: '李明', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=user1', verified: true, title: 'Python开发者' },
    { id: 2, name: '王芳', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=user2', verified: false, title: 'Web工程师' },
    { id: 3, name: '刘强', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=user3', verified: true, title: '全栈开发' },
  ];

  const twitterUsers = [
    { id: 11, name: '@pythondev', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=tw1', verified: true, followers: '10k' },
    { id: 12, name: '@webmaster', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=tw2', verified: false, followers: '5k' },
  ];

  const facebookUsers = [
    { id: 21, name: 'Python学习小组', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=fb1', verified: false, members: '2.5k' },
    { id: 22, name: '编程爱好者', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=fb2', verified: true, members: '8k' },
  ];

  const getCurrentUserList = () => {
    switch (inviteTab) {
      case 'platform': return platformUsers;
      case 'twitter': return twitterUsers;
      case 'facebook': return facebookUsers;
      default: return [];
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.tabBar}>
        <TouchableOpacity
          style={[styles.tab, inviteTab === 'platform' && styles.tabActive]}
          onPress={() => {
        ...
        <TouchableOpacity
          style={[styles.tab, inviteTab === 'twitter' && styles.tabActive]}
          onPress={() => {
        ...
        <TouchableOpacity
          style={[styles.tab, inviteTab === 'facebook' && styles.tabActive]}
          onPress={() => {
...
const styles = StyleSheet.create({
  ...
  selectedCountText: { fontSize: 13, color: '#f59e0b', fontWeight: '500', textAlign: 'center' },
  userList: { flex: 1, paddingHorizontal: 16 },
  userItem: { 
    flexDirection: 'row', 
    alignItems: 'center', 
    justifyContent: 'space-between', 
  ...
  userItemSelected: { backgroundColor: '#fef3c7', borderWidth: 1, borderColor: '#fcd34d' },
  userLeft: { flexDirection: 'row', alignItems: 'center', flex: 1, gap: 12 },
  userInfo: { flex: 1 },
  userNameRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginBottom: 4 },
  userName: { fontSize: 14, fontWeight: '500', color: '#1f2937' },
  userMeta: { fontSize: 12, color: '#9ca3af' },
  checkbox: { 
    width: 24, 
    height: 24, 
    borderRadius: 12, 
  ...
  sendBtnDisabled: { backgroundColor: '#fcd34d', opacity: 0.5 },
  sendBtnText: { fontSize: 15, color: '#fff', fontWeight: '600' },
});
