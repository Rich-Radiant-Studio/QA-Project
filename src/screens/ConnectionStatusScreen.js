 ...
 * 连接状态检查页面
 */
export default function ConnectionStatusScreen({ navigation }) {
  ...
  const checkAllStatus = async () => {
    ...
    try {
      // 并行检查所有状态
      const [networkStatus, backendStatus, metroStatus] = await Promise.all([
        checkNetworkStatus(),
        checkBackendStatus(),
        checkMetroStatus(),
      ]);
          ...
          {details.map((detail, index) => (
            <View key={index} style={styles.detailRow}>
              ...
              <Text style={[
                ...
                detail.error && styles.detailValueError,
              ]}>
                {detail.value}
              </Text>
            </View>
      ...
      <View style={styles.header}>
        ...
        >
          <Ionicons
            name="refresh"
            size={24}
            color={refreshing ? '#9ca3af' : '#1f2937'}
          />
        </TouchableOpacity>
      </View>
      ...
      >
        ...
        <View style={styles.overallStatus}>
          <View style={styles.overallStatusIcon}>
            {status.network?.connected && status.backend?.connected ? (
        ...
        {renderStatusCard(
          ...
          [
            ...
            {
              label: '响应时间',
              value: status.backend?.responseTime
                ? `${status.backend.responseTime}ms`
                : '-',
            },
            ...(status.backend?.error
              ? [{ label: '错误', value: status.backend.error, error: true }]
              : []),
          ]
        )}
        ...
        {(!status.network?.connected || !status.backend?.connected) && (
          <View style={styles.troubleshootCard}>
            ...
            {!status.backend?.connected && status.network?.connected && (
              <View style={styles.troubleshootItem}>
                <Text style={styles.troubleshootText}>
