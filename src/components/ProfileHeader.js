 ...
 */
const VerificationIcon = ({ size = 16, color = '#3b82f6' }) => {
  return (
    <View 
      style={{
        width: size,
...
function ProfileHeader({ userData, onStatPress, isFollowing, onFollowPress, isOwnProfile }) {
  ...
  const getVerificationInfo = () => {
    ...
    switch (verification.type) {
      case 'personal':
        return { icon: 'checkmark-circle', color: '#3b82f6' };
      case 'enterprise':
        return { icon: 'business', color: '#f59e0b' };
      case 'government':
        ...
        <View style={styles.leftSection}>
          ...
          <View style={styles.statsRow}>
            <TouchableOpacity 
              ...
              onPress={() => onStatPress && onStatPress('likes')}
            >
            ...
            <TouchableOpacity 
              style={styles.statItem}
              onPress={() => onStatPress && onStatPress('friends')}
            >
          ...
          {verification && verification.verified && (
            <View style={styles.verificationRowCompact}>
...
const styles = StyleSheet.create({
  ...
  avatarWrapper: {
    ...
    bottom: -scaleSpacing(22), // 调整位置（66/3 = 22，下方占1/3）
    zIndex: 2,
  },
  avatarContainer: {
    position: 'relative',
    borderWidth: 3,
    borderColor: '#fff',
    borderRadius: scaleSpacing(36), // (66 + 3*2) / 2 = 36
    backgroundColor: '#fff',
  },
  // 小型关注按钮（在头像右下角）
  followButtonSmall: {
